...
public class AuthService {
    ...
    public TokenResponse refresh(TokenRefreshRequest request) {
        ...
        if (!refreshTokenStore.isTokenValid(userId, tokenId)) {//在redis中查找刷新令牌
            throw new BusinessException(ErrorCode.REFRESH_TOKEN_INVALID);
        }

        User user = findUserById(userId).orElseThrow(() -> new BusinessException(ErrorCode.IDENTIFIER_NOT_FOUND));
        TokenPair tokenPair = jwtService.issueTokenPair(user);//签发新的令牌对
    ...
    public void resetPassword(PasswordResetRequest request) {
        ...
        ensureVerificationSuccess(verificationService.verify(VerificationScene.RESET_PASSWORD, identifier, request.code()));//验证码校验
        user.setPasswordHash(passwordEncoder.encode(request.newPassword().trim()));
        userService.updatePassword(user);//更新密码(数据库显示密文)
        refreshTokenStore.revokeAll(user.getId());//撤销所有刷新令牌
    }
